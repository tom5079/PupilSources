...
suspend fun HttpClient.getGalleryBlock(galleryID: Int) : GalleryBlock {
    val info = getGalleryInfo(galleryID)

    return GalleryBlock(
        galleryID,
        "",
        listOf(urlFromUrlFromHash(info.files.first(), "webpbigtn", "webp", "tn")),
        info.title,
        info.artists.orEmpty().map { it.artist },
        info.parodys.orEmpty().map { it.parody },
        info.type,
        info.language.orEmpty(),
        info.tags.orEmpty().map { it.toString() },
        info.groups.orEmpty().map { it.group }
    )
}